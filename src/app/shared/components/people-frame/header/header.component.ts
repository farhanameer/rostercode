...
import { ConfigureNotificationService } from "src/app/modules/nms/services/configure-notifications.service";
import { ObservableService } from "src/app/modules/nms/util/observablefn.service";
...
export class HeaderComponent implements OnInit {
  ...
  constructor(
    ...
    private cns: ConfigureNotificationService,
  ...
  getMenus(link: string) {
    ...
    this.headerService.getMenus(link).subscribe((result: Array<any>) => {
      ...
      if (fileName) {
        ...
        const portal =
          ...
          localStorage.getItem("link") != "null"
            ...
            : "";
        const employeeId =
          typeof localStorage.getItem("e_number") != "undefined" &&
          localStorage.getItem("e_number") != "null"
        ...
        this.authService
          ...
          .subscribe((data) => {
            if (data.status == "200" && data.loginId != "false") {
              let routingLink =
                environment.phpSiteUrl + "?u=" + data.loginId + "&mt=aes";
  ...
  getPortalName(): string {
    if (this.portals)
      return this.portals.filter(
        (map) =>
          map.group_name === this.selectedPortal ||
          map.link === this.selectedPortal
      )[0].name;
    return "";
  }

  onLogout() {
    const userId = localStorage.getItem("device_id");
    this.os.asPromised(this.cns.deleteDevice(userId)).then((res) => {
